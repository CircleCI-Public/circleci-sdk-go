...
func TestEnvService_List(t *testing.T) {
	...
	envService := env_project.NewEnvService(c)
	...
	t.Run("list", func(t *testing.T) {
		...
		assert.Assert(t, err)
		assert.Check(t, cmp.DeepEqual(envs, []env_project.EnvVariable{
			{
				Name:      "FIREBASE_TOKEN",
				CreatedAt: time.Now(),
			},
...
func TestEnvService_Create(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		assert.Assert(t, err)
		assert.Check(t, cmp.DeepEqual(envCreated, &env_project.EnvVariable{
			Name:      "test_sdk",
			CreatedAt: time.Now(),
			Value:     "VALUE",
		}, cmpopts.EquateApproxTime(time.Second)))
