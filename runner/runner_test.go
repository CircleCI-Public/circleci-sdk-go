...
func TestClaimUnclaim(t *testing.T) {
	...
	// Create a resource class
	createReq := CreateResourceClassRequest{
		ResourceClass: "test-org/test-claim-unclaim",
		Description:   "Test claim/unclaim",
	}
	resourceClass, err := service.CreateResourceClass(ctx, createReq)
	assert.Assert(t, err)

	// Attempt to claim a task (may return 204 if no tasks available)
	claimReq := ClaimTaskRequest{
		Hostname: "test-hostname",
		IP:       "127.0.0.1",
		Name:     "test-runner",
		Version:  "1.0.0",
	}
	response, err := service.ClaimTask(ctx, claimReq)
	// Note: This may fail if no tasks are available, which is expected
	// The test verifies the API call structure is correct
	if err == nil && response != nil {
		// If we got a task, try to unclaim it
		if response.TaskToken != "" {
			unclaimReq := UnclaimTaskRequest{
				TaskId:    "test-task-id",
				TaskToken: response.TaskToken,
			}
			_ = service.UnclaimTask(ctx, unclaimReq)
			// Unclaim may fail if the task was not actually valid, which is ok for this test
		}
	}
